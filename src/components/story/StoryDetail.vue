<template>
    <div class="modal-overlay" @click.stop="closeModal">
      <v-card class="card" ref="modalContent">
        <span class="title1">
          <img
            :src="story.userImage ? story.userImage : '/src/assets/main/user.png'"
            class="user-image"
          />
          <span>{{ story.nickname }}</span>
          <span class="date"> {{ computedDate(createdAt) }}</span>
          <span>날씨</span><img width="30px" :src="computedWeather">
          <span>감정</span><img width="30px" :src="computedEmotion">
          ...
          <div class="right-content">
            ...
            <div v-for="comment in comments" :key="comment.storyCommentId">
              ...
              <div class="storyComment">
                <img
                  width="10px"
                  class="storyComment_img"
                  :src="comment.userImage ? comment.userImage : '/src/assets/main/user.png'"
                >
              ...
              <div class="reply" v-for="reply in comment.replies" :key="reply.storyReplyId">
                ...
                <img
                  width="10px"
                  class="storyComment_img"
                  :src="reply.userImage ? reply.userImage : '/src/assets/main/user.png'"
                >
                <p>{{ reply.nickname }}</p>
              </div>
